...
Graph* GraphGetSubgraph(const Graph* g, IndicesSet* vertSet) {
  ...
  while (v != -1) {
    ...
    struct _Vertex* orig_vertex = ListGetCurrentItem(g->verticesList);
    List* edges = orig_vertex->edgesList;
    if (!ListIsEmpty(edges)) {
      ListMoveToHead(edges);
      for (int i = 0; i < ListGetSize(edges); ListMoveToNext(edges), i++) {
        struct _Edge* edge = ListGetCurrentItem(edges);
        unsigned int adj = edge->adjVertex;
        if (!IndicesSetContains(vertSet, (uint16_t)adj)) {
          continue;
        }
        if (g->isDigraph || orig_vertex->id < adj) {
          if (g->isWeighted) {
            GraphAddWeightedEdge(new, orig_vertex->id, adj, edge->weight);
          } else {
            GraphAddEdge(new, orig_vertex->id, adj);
          }
        }
      }
    }
...
double* GraphComputeVertexWeights(const Graph* g) {
  ...
  ListMoveToHead(vertices);
  unsigned int idx = 0;
  for (; idx < g->numVertices; ListMoveToNext(vertices), idx++) {
    struct _Vertex* vertex = ListGetCurrentItem(vertices);
    if (g->isWeighted == 0) {
      weightsArray[vertex->id] = (double)vertex->outDegree;
      continue;
    }
    ...
    weightsArray[vertex->id] = weight_sum;
  }
